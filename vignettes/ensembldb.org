...
Alternatively, the =ensDbFromGtf= builds an EnsDb SQLite file from an Ensembl GTF file and thus doesn't depend on the Ensembl Perl API.
...
** TODO Use the =GenomicFeatures= =fetchChromLengthsFromEnsembl= to retrieve chromosome lengths for GTF import
